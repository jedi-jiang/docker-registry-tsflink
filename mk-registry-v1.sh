#!/bin/bash
BUILDDIR=`mktemp -d /tmp/cn_tsflink_reg_v1.XXXXX` || $(echo "Create build directory fail!" && exit 1)
cd "$BUILDDIR"

cat > config_tsflink.yml <<"EOF"
...
prod:
    <<: *local
    search_backend: _env:SEARCH_BACKEND:sqlalchemy
    sqlalchemy_index_database: _env:SQLALCHEMY_INDEX_DATABASE:sqlite:////data/db/registry.db
...
# DESCRIPTION:   Image of the docker-registry for production running in the TSFLINK.
# TO_BUILD:      docker build -rm tsflink-registry .
# TO_RUN:        docker run -p 5000:5000 tsflink-registry

FROM registry:0.9.1

RUN mkdir -p /data/{db,registry}

COPY config_tsflink.yml /docker-registry/config/

ENV DOCKER_REGISTRY_CONFIG /docker-registry/config/config_tsflink.yml
...
docker rmi udiabon/docker-registry-v1-tsflink:latest
docker build --rm -t udiabon/docker-registry-v1-tsflink .
